...
export const workflowLoader = {
  ...
  async loadConfig(configId: string, options: any = {}) {
    try {
      ...
      if (config.agents?.browser_extension) {
        if (options.agentUrl) config.agents.browser_extension.url = options.agentUrl;
        if (options.bootUrl) config.agents.browser_extension.boot_url = options.bootUrl;
        if (options.passcode) config.agents.browser_extension.passcode = options.passcode;
      }
  ...
  async generatePasscodeAndConfig() {
    try {
      ...
      const config = this.getDefaultConfig('create-client-config', { passcode });
  ...
  parseYaml(yamlText: string) {
    try {
      ...
      for (const line of lines) {
        ...
        else if (trimmed.endsWith(':') && line.startsWith('  ') && !line.startsWith('    ')) {
